...
description: 剪映 (JianYing) AI自动化剪辑的高级封装 API。支持录屏、素材导入、字幕生成、Web 动效合成及项目导出。
version: 1.0.0
metadata:
  openclaw:
    requires:
...
- 录屏与智能变焦：`rules/recording.md` -> `tools/recording/recorder.py` (Windows) / `tools/recording/macos_recorder.py` (macOS)
